...
class Block(object):
    ...
    def get_fillings(self):
        ...
        for rect in self.shape:
            x = (rect[0] - constants.BOARD_MARGIN_LEFT) // constants.BWIDTH
            y = (rect[1] - constants.BOARD_MARGIN_TOP) // constants.BHEIGHT
            output.append((x, y))
        return output
    def __str__(self):
    ...
    def get_min(self):
        fillings = self.get_fillings()
        y = [x[1] for x in fillings]
        return np.min(y)
    def draw(self):
        """
