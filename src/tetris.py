...
class Tetris(object):
    ...
    def get_game_state(self):
        board_state = np.zeros((constants.BOARD_BLOCK_HEIGHT, constants.BOARD_BLOCK_WIDTH))
        for blk in self.blk_list:
            if blk == self.active_block:
                value = 2
            else:
                value = 1
            coord_list = blk.get_fillings()
            for coords in coord_list:
                board_state[coords[1] - 1][coords[0] - 1] = value
        return board_state

    def get_game_state_flat(self):
        board_state = np.zeros(constants.BOARD_BLOCK_HEIGHT * constants.BOARD_BLOCK_WIDTH)
        for blk in self.blk_list:
            if blk == self.active_block:
                value = 2
            ...
            coord_list = blk.get_fillings()
            for coords in coord_list:
                board_state[coords[1] * constants.BOARD_BLOCK_WIDTH + coords[0]] = value
        return board_state
    ...
    def env_game_logic(self, action):
        reward = 0
    ...
    def detect_line(self):
        ...
        min_row = end
        if min_row - self.active_block.get_min() <= 1:
            ret_score = 1
        else:
            ret_score = -1
        # ret_score = min_row - self.active_block.get_min()
        # print(f"min_row: {min_row}, minimum: {self.active_block.get_min()}, ret_score: {ret_score}")
    ...
    def get_block(self):
        ...
        if self.new_block:
            ...
            self.active_block = block.Block(data[0], self.start_x, self.start_y, None, data[1],
                                                       data[2])
